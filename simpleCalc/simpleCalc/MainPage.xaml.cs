...
namespace simple_calc
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        {
            textBoxCalc.Text += ((Button)sender).Content;            
        }
        //  演算子ボタン
        ...
        //  クリア(C)ボタン
        private void Clear_Click(object sender, RoutedEventArgs e)  //  式の初期化
        {
            textBoxCalc.Text = String.Empty;
            init_flag = true;            
        }

        private async void eq_Click(object sender, RoutedEventArgs e)  //  式の計算を行う
        {
            int next_op_pos = 0;
            int before_op_pos = 0;
            char ch = ' ';
            ...
            if (textBoxCalc.Text == "=")
            {
                MessageDialog msg = new MessageDialog("式を入力してください。", "エラー");
                textBoxCalc.Text = String.Empty;
                await msg.ShowAsync();
                return;
            }
            if (textBoxCalc.Text == "-")
            {
                MessageDialog msg = new MessageDialog("入力した文字は不正です。", "エラー");
                textBoxCalc.Text = String.Empty;
                await msg.ShowAsync();
                return;
            }
